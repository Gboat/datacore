...
<div class="left_s">
<?php if(!$pageOnOff) { ?>
<b>当前<?php echo $member['fans_count']; ?>人在关注<?php echo $member['nickname']; ?> </b>
<?php if(MEMBER_ID) { ?>
（<span><a href="index.php?mod=profile&code=invite" target="_blank">邀请好友</a>来关注</span>）
<?php } ?>
<?php } else { ?><div>
<a href="index.php?mod=admin&code=fans">返回关注我的人</a><em> &gt; </em>昵称含"<?php echo $nickname; ?>"的人（<?php echo $followcount; ?>）
</div>
<?php } ?>
</div>
	<!--搜索关注 start -->
...
<?php $serachValue = $keyword ? $keyword : "请输入关键字"; ?>
   <input class="sc_r_t_a" id="keywarod" name="headSearchValue" type="text" value="<?php echo $serachValue; ?>" onfocus="this.value=''" />
...
<div class="nfBox" style="margin:0 auto">
  <div class="left">排序方式：
<?php if(empty($this->Get['type'])) $_follow_time = "current"; ?>
<?php if($this->Get['type'] == 'lastpost') 	$lastpost = "current"; ?>
<?php if($this->Get['type'] == 'fans_count') $fans_count = "current"; ?>
<a href="index.php?mod=<?php echo $this->Get['mod_original']; ?>&code=<?php echo $this->Code; ?>" class="<?php echo $_follow_time; ?>">关注时间</a>
	<a href="index.php?mod=<?php echo $this->Get['mod_original']; ?>&code=<?php echo $this->Code; ?>&type=lastpost" class="<?php echo $lastpost; ?>">最近活跃</a>
	<a href="index.php?mod=<?php echo $this->Get['mod_original']; ?>&code=<?php echo $this->Code; ?>&type=fans_count" class="<?php echo $fans_count; ?>">粉丝数</a>
  </div>
  <div class="clear"></div>
</div>
<!--粉丝筛选 End-->
...
<li class="pane" id="fans_user_<?php echo $val['uid']; ?>">
<div class="fBox_l ">
<a href="index.php?mod=<?php echo $val['username']; ?>"><img onerror="javascript:faceError(this);" src="<?php echo $val['face']; ?>" onmouseover="get_user_choose(<?php echo $val['uid']; ?>,'_user',<?php echo $val['uid']; ?>)" onmouseout="clear_user_choose()"/></a>
</div>
<div id="user_<?php echo $val['uid']; ?>_user" class="layS"></div>
<div class="fBox_R ">
		 <p style="width:330px; float:left;">        
         <span class="name" style="float:left; display:inline;"><a href="index.php?mod=<?php echo $val['username']; ?>"><?php echo $val['nickname']; ?></a></span>
...
<?php } ?>
<?php if($val['gender'] == 1) { ?>
         <img src="templates/default/images/user.gif"  title="<?php echo $val['nickname']; ?>"/>
         <?php } elseif($val['gender'] == 2) { ?>         <img src="templates/default/images/user_female.gif"  title="<?php echo $val['nickname']; ?>"/>
<?php } ?>
		 <?php echo $this->hookall_temp['global_fans_extra1']; ?>
<?php if(MEMBER_ID==$member['uid']) { ?>
		 <span class="fBox_R_remarklist">
         <a href="javascript:void(0)" onclick="getElementById('add_remark_<?php echo $val['uid']; ?>').style.display='';getElementById('remark_<?php echo $val['uid']; ?>').value='<?php echo $val['remark']; ?>';">
         <i id="remarklist_<?php echo $val['uid']; ?>">
<?php if($val['remark']) { ?>
(<?php echo $val['remark']; ?>)
<?php } ?>
</i>
...
<?php } ?>
		...
		<?php echo $this->hookall_temp['global_fans_extra2']; ?>
		</div>
...
<br />
<?php if(MEMBER_ID==$member['uid']) { ?>
		<a href="javascript:void(0)" onclick="follower_choose(<?php echo $val['uid']; ?>,'<?php echo $val['uid']; ?>','del_fans','');">移除粉丝</a>
<?php } ?>
</div>
...
<li>所有关注<?php echo $member['username']; ?>的人，都能在其个人首页中第一时间看到其发布的信息。<BR>
通过发布或者转发他人的微博，将会吸引更多的人关注
<?php if(MEMBER_ID > 0) { ?>
...
</div><?php /* 2013-01-26 in jishigou invalid request template */ if(!defined("IN_JISHIGOU")) exit("invalid request"); ?><div class="mainR">
