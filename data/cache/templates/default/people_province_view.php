...
<div class="main_2b">
  <div class="main_t26">
    <div class="main_t261">名人堂 > <b><?php echo $categoryname; ?></b></div>
...
<?php if(is_array($member_list)) { foreach($member_list as $val) { ?>
            <div class="pf2 selected crPer" alt="<?php echo $val['vip_info']; ?>">
                <p class="mark"></p>
                <p class="avt">
...
<?php } ?>
                ...
                <b class="nm"><a href="index.php?mod=<?php echo $val['uid']; ?>" target="_blank" title="<?php echo $val['vip_info']; ?>"><?php echo $val['nickname']; ?></a></b> 
            </div> 
            <div id="user_<?php echo $val['uid']; ?>_user"></div>
<?php } } ?>
</div>
    <div class="categories_menulist">
<?php if(is_array($province_ary)) { foreach($province_ary as $val) { ?>
        <a href="index.php?mod=people&code=province&pid=<?php echo $val['id']; ?>"> 
<?php if($categoryname == $val['name']) { ?>
<strong><?php echo $val['name']; ?></strong>
<?php } else { ?><?php echo $val['name']; ?>
<?php } ?>
</a>
<?php } } ?>
      </div>
...
<?php } } ?>
  ...
  <div class="mediaBox">
<?php if(is_array($city_ary)) { foreach($city_ary as $city_val) { ?>
<?php if($city_val['user_count']) { ?>
      <div class="famTabletop"></div>
        <div class="famTablecen">
          <div class="titleBar">
            <div class="lf"><a href="index.php?mod=people&code=city&pid=<?php echo $city_val['upid']; ?>&cid=<?php echo $city_val['id']; ?>"><?php echo $city_val['name']; ?></a></div>
            <div class="rt">
              <label style="cursor:pointer;"><input type="checkbox"  onClick="reverseCheck(follow_form_<?php echo $city_val['id']; ?>);" />全选 </label> |
              <span 
<?php if(MEMBER_ID <1 ) { ?>
 onclick="ShowLoginDialog(); return false;" 
<?php } ?>
>  
              <input type="submit" class="shareI" name="follow_Submit"  value="加关注" onclick="document.getElementById('follow_form_<?php echo $city_val['id']; ?>').submit();"/>
              </span>
            </div>
          </div>
          <iframe id="user_follow_frame_<?php echo $city_val['id']; ?>" name="user_follow_frame_<?php echo $city_val['id']; ?>" width="0" height="0" marginwidth="0" frameborder="0" src="about:blank"></iframe>
            <form method="POST"  action="ajax.php?mod=topic&code=add_user_follow&__to=iframe" id="follow_form_<?php echo $city_val['id']; ?>" name="follow_form_<?php echo $city_val['id']; ?>" target="user_follow_frame_<?php echo $city_val['id']; ?>">
<input type="hidden" name="FORMHASH" value='<?php echo FORMHASH; ?>'/>
            <div class="ulist6" style="width:740px;"> 
<?php if(is_array($members)) { foreach($members as $val) { ?>
<?php if($city_val['name'] == $val['city']) { ?>
                    <div class="pf2 selected crPer">
                        <p class="status">
                        <input type="checkbox" id="sid_<?php echo $val['uid']; ?>" name="uids[]" value="<?php echo $val['uid']; ?>"/>
                        </p> 
                        <p class="mark"></p>
                        <p class="avt"><b><label for="sid_<?php echo $val['uid']; ?>" style="cursor:pointer;"><img src="<?php echo $val['face']; ?>" onerror="javascript:faceError(this);"onmouseover="get_user_choose(<?php echo $val['uid']; ?>,'_user',<?php echo $val['uid']; ?>)" onmouseout="clear_user_choose()"/></label></b></p> 
