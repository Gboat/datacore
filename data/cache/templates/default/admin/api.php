...
<?php } ?> <br />
<?php if('modify'==$this->Code) { ?>
<form method="post"  action="admin.php?mod=api&code=do_modify">
<input type="hidden" name="FORMHASH" value='<?php echo FORMHASH; ?>'/><input
	type="hidden" name="id" value="<?php echo $app['id']; ?>" />
<table cellspacing="1" cellpadding="4" width="100%" align="center"
	...
	<tr class="altbg1">
		...
		<td><?php echo $app['app_key']; ?></td>
	</tr>
	<tr class="altbg2">
		<td width="40%"><b>APP SECRET:</b> <span class="smalltxt"></span><br />
		</td>
		<td><?php echo $app['app_secret']; ?></td>
	</tr>
	<tr class="altbg1">
		<td width="40%"><b>应用名称:</b> <span class="smalltxt"></span><br />
		</td>
		<td><input type="text" name="app_name" value="<?php echo $app['app_name']; ?>"
	...
	<tr class="altbg2">
		...
		<td><input type="text" name="source_url"
			value="<?php echo $app['source_url']; ?>" style="width: 300px;" /></td>
	</tr>
	<tr class="altbg1">
		<td width="40%"><b>应用介绍:</b> <span class="smalltxt"></span><br />
		</td>
		<td><textarea name="app_desc" style="width: 300px;"><?php echo $app['app_desc']; ?></textarea></td>
	</tr>
	<tr class="altbg2">
		<td width="40%"><b>显示来自详情？</b> <span class="smalltxt">选择“是”，微博页面会显示来自具体的API应用名称和相应的网址
		</span><br />
		</td>
		<td><?php echo $app_show_from_radio; ?></td>
	</tr>
	<tr class="altbg1">
		<td width="40%"><b>开启本API应用？</b> <span class="smalltxt"></span><br />
		</td>
		<td><?php echo $app_status_radio; ?></td>
	</tr>
</table>
<br />
...
<?php } else { ?><form method="post"  action="admin.php?mod=api&code=do_modify_setting">
...
<table cellspacing="1" cellpadding="4" width="100%" align="center"
	...
	<tr class="altbg1">
		...
		<td><?php echo $app_enable_radio; ?> <a target="_blank"
	...
	<tr class="altbg2">
		...
		<td><?php echo $app_from_enable_radio; ?></td>
	</tr>
	<tr class="altbg1">
		...
		<td><input type="text" name="api[request_times_day_limit]"
			value="<?php echo $api['request_times_day_limit']; ?>" /></td>
	</tr>
</table>
<br />
<center><input class="button" type="submit" name="cronssubmit"
...
<?php } ?>
<?php if(is_array($app_list)) { foreach($app_list as $app) { ?>
	<tr align="center" class="altbg2">
		<td>
<?php if($app['source_url']) { ?>
 <a target="_blank"
			href="<?php echo $app['source_url']; ?>"><b><?php echo $app['app_name']; ?></b></a> 
<?php } else { ?><?php echo $app['app_name']; ?> 
<?php } ?>
</td>
		<td><?php echo $app['app_key']; ?><br />
		<?php echo $app['app_secret']; ?></td>
		<td><?php echo $app['last_request_time_html']; ?></td>
		<td>
		<?php echo $app['request_times_day']; ?>/<?php echo $app['request_times_week']; ?>/<?php echo $app['request_times_month']; ?>/<?php echo $app['request_times_year']; ?>/<?php echo $app['request_times']; ?><br />
		<?php echo $app['request_times_last_day']; ?>/<?php echo $app['request_times_last_week']; ?>/<?php echo $app['request_times_last_month']; ?>/<?php echo $app['request_times_last_year']; ?>
		</td>
		<td><?php echo $app['status_html']; ?></td>
		<td>
<?php if($app['status']) { ?>
 <a
...
<a onclick="return confirm('删除后的内容不可恢复，确认删除？');"
			...
			href="admin.php?mod=api&code=modify&id=<?php echo $app['id']; ?>">编辑</a> &nbsp;</td>
	</tr>
<?php } } ?>
<tr class="altbg1">
