 ...
 *
 * @Date 2012-04-28 05:53:03 1877599680 1012640721 20210 $
 *******************************************************************/
...
class EventLogic
{
	...
	function getEventInfo($param){
		...
		if ($count) {
	    	...
	    	$sql = "select a.id,a.title,a.fromt,a.tot,a.need_app_info,a.item,a.item_id, 
	    			   ...
	    			   a.postman,a.posttime,m.nickname,m.username,m.uid,am.store,am.app 
	...
	function getEvents($param){
		...
		$event_list = array();
		$count_sql = "select count(*) from ".TABLE_PREFIX."event_member m where $where_sql ";
		$count = DB::result_first($count_sql);
	...
	function getAllUser($param){
	    ...
	    $sql = "select m.uid,m.nickname,m.username,m.face,m.province,m.city,a.app_time,a.app_info 
