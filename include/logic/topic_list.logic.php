 ...
 *
 * @Date 2012-04-28 05:53:04 133956017 1317525842 26978 $
 *******************************************************************/
...
class TopicListLogic
{
	...
	function get_data($param, $caller = "web")
	{
		...
		if ($total_record < 1) {
			if(!$cache_time || (false === ($total_record = Load::model('cache/db')->get($cache_key . '-total_record')))) {
				$sql = " select count(*) from ".DB::table("topic")." {$where_sql} ";
				$total_record = DB::result_first($sql);

				if($cache_time) {
					Load::model('cache/db')->set($cache_key . '-total_record', $total_record, $cache_time);
				}
			}
		}
		...
		if (defined('NEDU_MOYO'))
		{
			...
			if(!$cache_time || (false === ($topic_list = Load::model('cache_db')->get($cache_key . '-topic_list-' . $limit_sql)))) {
								$topic_list = $this->TopicLogic->Get($condition, $fields, $proc_func, $table);
				if (empty($topic_list)) {
					return false;
				}
				if($cache_time) {
					Load::model('cache/db')->set($cache_key . '-topic_list-' . $limit_sql, $topic_list, $cache_time);
				}
			}
	...
	{		
		...
		if(false === ($info = Load::model('cache/db')->get($cache_key))) {
			if($uid > 0) {
			...
			if ($total_photo > 0) {
				...
				if($param['vip']) {
					$query = DB::query("SELECT t.id,t.tid,t.uid,t.width,t.height,t.dateline,tr.content,tr.forwards,tr.replys
				...
				if($topic_list) {
					...
					$info = array(
						...
						Load::model('cache/db')->set($cache_key, $info, ($uid > 0 ? 3600 : 600));
		}
	...
	function get_options($options=array(), $cache_time=600, $cache_key='') {
		$cache_time = max(0, (int) $cache_time);
			...
			if(false === ($_cache_info=Load::model('cache/db')->get($cache_key))) {
				$_options = $options;
				...
				Load::model('cache/db')->set($cache_key, $_cache_info, $cache_time);
			}
