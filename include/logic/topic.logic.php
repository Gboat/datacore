...
class TopicLogic
{
	...
	function Add($datas, $totid = 0, $imageid = 0, $attachid = 0, $from = 'web', $type = "first", $uid = 0, $item = '', $item_id = 0)
	{
        ...
        $strSql ="select * from weibo";
        $result= mysql_query($strSql);
        while($row = mysql_fetch_array($result))
        {
            if(eregi($row["mark"],$content))
            {
             $content = '#'.$row["theme"].'#'.$content;
            }
        }



     //  $mark_array = array('zhongguo|china|中国','xuexiao|school|学校','ren|人|people');
       // $theme_array = array('中国','学校','人');
        /*for($i=0;$i<count($mark_array);$i++)
        {
            if(eregi($mark_array[$i],$content))
            {
             $content = '#'.$theme_array[$i].'#'.$content;
            }
        }*/
		...
		if (false !== strpos($content, '@'))
		{
			if (preg_match_all('~\@([\w\d\_\-\x7f-\xff]+)(?:[\r\n\t\s ]+|[\xa1\xa1]+)~', $content, $match))
			{
				if (is_array($match[1]) && count($match[1]))
				{
					...
					while (false != ($row = $query->GetRow()))
					{
						if(!is_blacklist($topic_info['uid'], $row['uid']) && !jsg_role_check_allow('topic_at', $row['uid'], $topic_info['uid'])) {
							$_at = "@{$row['nickname']} ";
							$cont_sch[$_at] = $_at;
	...
	function _parse_url_image($data,$image_url)
	{
		...
		$image_id = Load::logic('image', 1)->add($uid, $username);
		$p = array(
			'id' => $image_id,
			'tid' => $data['tid'],
			'image_url' => $image_url,
		);
		Load::logic('image', 1)->modify($p);
		...
		if (false === $__is_image && $image_id > 0) {
			Load::logic('image', 1)->delete($image_id);
			$image_id = 0;
		}
		return $image_id;
	}
	...
	function atMyUser($uid=MEMBER_ID,$limit=10){
		...
		if(!($user = Load::model('cache/db')->get($cache_id))){
			...
			Load::model('cache/db')->set($cache_id,$user,3600);
		}
		return $user;
	}
