...
class passport {
    ...
    function checkname($username, $is_nickname = 0, $ucuid = 0, $check_exists = 1) {
        $username = trim(strip_tags($username));
        $username_len = $this->jstrlen($username);
    ...
    function delete($ids) {
        ...
        if($member_ids_count > 0)
        {
            ...
            $tbs = array(
                ...
                'event_member' => 'fid',
                'fans_group' => 'uid',
                'fans_group_fields' => array('uid', 'touid'),
                'group' => 'uid',
                'groupfields' => 'uid',
        ...
        if($email && $email!=$oldmember['email'])
        {
            ...
            if($GLOBALS['_J']['config']['reg_email_verify'])
            {
                $newmember['role_id'] = $GLOBALS['_J']['config']['no_verify_email_role_id'];
                                Load::functions('my');
                my_member_validate($oldmember['uid'],$email,(int) ($newmember['role_id']!=$GLOBALS['_J']['config']['no_verify_email_role_id'] ? $oldmember['role_id'] : $GLOBALS['_J']['config']['normal_default_role_id']));
            }
