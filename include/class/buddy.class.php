...
class buddy {
	...
	function get($p, $cache_time=0) {
		...
		if($cache_time) {
			...
			if(false !== ($rets = Load::model('cache/db')->get($cache_id))) {
				return $rets;
			}
		}
		...
		if($cache_time) {
			Load::model('cache/db')->set($cache_id, $rets, $cache_time);
		}
	...
	function count($uid) {
		...
		if($follow_count != $member['follow_count']) {
			...
			Load::model('cache/db')->del("{$uid}-buddyids-%", 1);
	...
	function get_buddyids($uid, $uptime_limit=0) {
		...
		Load::model('cache/db')->set($cache_id, $ret, 3600);
	...
	function follow_html($member_list, $uid_field='uid', $follow_func='follow_html') {
		...
		foreach($member_list as $v) {
			if(isset($v['follow_html']) || !isset($v[$uid_field])) {
				return $member_list;
			}
		...
		foreach($member_list as $k=>$v) {
			$uid = $v[$uid_field];
			$isf = (isset($buddyids[$uid]) ? 1 : 0);
			$member_list[$k]['is_follow'] = $isf;
			$member_list[$k]['follow_html'] = $follow_func($uid, $isf);
		}
