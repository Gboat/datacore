...
{    
    ...
    $files = array(ROOT_PATH . 'setting/',ROOT_PATH . 'setting/settings.php',ROOT_PATH . 'setting/qqwb.php',ROOT_PATH . './data/cache/',);
    ...
    {
        $query = $db->query("select count(*) from ".TABLE_PREFIX."qqwb_bind_info",'SKIP_ERROR');
  ...
  UNIQUE KEY `qqwb_username` (`qqwb_username`)
        ...
        if (!$query) 
        {
            $msgs[] = "数据表<b>".TABLE_PREFIX."qqwb_bind_info</b>创建失败";
        }
        $query = $db->query("select count(*) from ".TABLE_PREFIX."qqwb_bind_topic",'SKIP_ERROR');
        if (!$query && '1146'==$db->errno()) 
        {
  ...
  KEY `qqwb_id` (`qqwb_id`)
        ...
        if (!$query) 
        {
            $msgs[] = "数据表<b>".TABLE_PREFIX."qqwb_bind_topic</b>创建失败";
        }            
    }        
    return $msgs;
}
