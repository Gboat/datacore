 ...
 * @version $Id: member.func.php 860 2012-04-27 02:15:13Z wuliyong $
 */
...
function jsg_member_get($p, $mark=1, $cache=1) {
	...
	if($count > 0) {
		...
		if($list) {
			if($mark) {
				$list = Load::model('buddy')->follow_html($list);
			}
...
function jsg_member_make($row) {	
	if (isset($row['uid'])) {
		...
		if($validate_id) {
			...
			$vip_pic = ($category_pic ? $category_pic : 'images/vip.gif');
...
function jsg_info($id, $table='', $pri='id', $cache_time = '-1') {
	...
	if(!$cache_time || false === ($info = Load::model('cache/file')->get($cache_id))) {
		$info = DB::fetch_first("SELECT * FROM ".DB::table($table)." WHERE `{$pri}`='$id'");

		if($cache_time) {
			Load::model('cache/file')->set($cache_id, $info, $cache_time);
		}
	}
...
function jsg_get_vip_uids($limit=300, $day=30) {
	...
	if(false === ($vip_uids = Load::model('cache/file')->get($cache_id))) {
		...
		Load::model('cache/file')->set($cache_id, $vip_uids, 600);
	}
