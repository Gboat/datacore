...
function debug($mixed,$halt=true)
{
    ...
    function referer($default = '?') {
        $ignore_domain = ($_POST['ignore_domain'] ? $_POST['ignore_domain'] : $_GET['ignore_domain']);
    ...
    function follow_html($uid,$follow=0,$addhtml=true) {
        ...
        if(MEMBER_ID>0 && MEMBER_ID!=$uid) {
            if ($follow) {
                $html = "<a href='javascript:void(0)' onclick=\"follow({$uid},'follow_{$uid}','');return false;\" class='follow_html2_2'></a>";
            } else {
                $html = "<a href='javascript:void(0)' onclick=\"follow({$uid},'follow_{$uid}','add');return false;\" class='follow_html2_1'></a>";
            }
