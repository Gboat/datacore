...
{    
	...
	$files = array(ROOT_PATH . 'setting/', ROOT_PATH . 'setting/settings.php', ROOT_PATH . 'setting/renren.php', ROOT_PATH . './data/cache/', );
	...
	{
		$query = $db->query("select count(*) from ".TABLE_PREFIX."renren_bind_info",'SKIP_ERROR');
		if (!$query && '1146'==$db->errno()) 
		{
			$query = $db->query("CREATE TABLE `".TABLE_PREFIX."renren_bind_info` (
  ...
  UNIQUE KEY `renren_uid` (`renren_uid`)
		...
		if (!$query) 
		{
			$msgs[] = "数据表<b>".TABLE_PREFIX."renren_bind_info</b>创建失败";
		}
		
		$query = $db->query("select count(*) from ".TABLE_PREFIX."renren_bind_topic",'SKIP_ERROR');
		if (!$query && '1146'==$db->errno()) 
		{
			$query = $db->query("CREATE TABLE `".TABLE_PREFIX."renren_bind_topic` (
  ...
  KEY `renren_id` (`renren_id`)
		...
		if (!$query) 
		{
			$msgs[] = "数据表<b>".TABLE_PREFIX."renren_bind_topic</b>创建失败";
		}
