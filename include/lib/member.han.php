 ...
 * @version $Id: member.han.php 793 2012-04-24 01:55:19Z wuliyong $
 */
...
class MemberHandler {
	...
	function HasPermission($mod, $code, $is_admin=0, $uid=0) {
		...
		if($uid) {
			...
			if($MemberFields && (false !== ($_role_info = Load::model('cache/file')->get("role/role_{$MemberFields['role_id']}"))) && is_array($_role_info) && count($_role_info)) {
		...
		if(!($this->ActionList[$mod])) {
			...
			if(false === ($cache_data = Load::model('cache/file')->get($cache_id))) {
				...
				Load::model('cache/file')->set($cache_id, array($action_list,$ActionList));
			} else {
		...
		if((($current_action_id=$this->ActionList[$mod]['index'][$action])!==null) || (($current_action_id=$this->ActionList[$mod]['index']["*"])!==null)) {
			...
			if($current_action['allow_all']=='-1') {
				...
				if($ActionList[$current_action_id]['message']) {
					...
					$message = "您的角色({$role_name})没有{$current_action['name']}权限；<br />如果您是(待验证会员)，请先通过<a href='index.php?mod=settings&code=base#modify_email_area'>邮件验证<a>或者<a href='index.php?mod=pm&code=send&folder=send'>联系管理员</a>";
				}
	...
	function GetMember() {
		...
		if($this->MemberFields['role_id'] < 1) {
			...
			if(false === ($role = Load::model('cache/file')->get($cache_id))) {
				$sql="SELECT
					`id` role_id,
					`name` role_name,
					`type` role_type,
	...
	function _getGuestRole() {
		...
		if(false === ($fields = Load::model('cache/file')->get($cache_id))) {
			$sql="SELECT
				`id` role_id,
				`name` role_name,
				`type` role_type,
			...
			Load::model('cache/file')->set($cache_id, $fields);
		}
		return $fields;
	}
