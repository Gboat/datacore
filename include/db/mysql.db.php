 ...
 * @version $Id: mysql.db.php 855 2012-04-27 01:43:23Z wuliyong $
...
class MySqlHandler extends DatabaseHandler
{
	...
	function SetTable($tableName,$skip_error=false)
	{
		...
		else
		{
			...
			if (false === ($fieldList = Load::model('cache/file')->get($cache_id)))
			{
				$sql = "SHOW \n\tCOLUMNS \nFROM \n\t`{$this->TableName}`";
				...
				Load::model('cache/file')->set($cache_id, $fieldList);
			}
