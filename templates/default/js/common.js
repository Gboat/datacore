...
// }
    ...
    else if( 'myblog'==r || 'myhome'==r || 'tagview'==r || 'new'==r  || 'qun'==r || 'event'==r || 'vote'==r || 'live'==r || 'talk'==r || 'ask'==r || 'answer'==r)		// 加入了对微群发微博的处理
    {
		...
		function (d) {
			...
			if(verify == 1){
				...
				if(''!=d) {
                    ...
                    videoid = 0;
					if( 'myblog'==r || 'myhome'==r || 'tagview'==r || 'new'==r || 'qun'==r || 'event'==r || 'vote'==r || 'live'==r || 'talk'==r || 'ask'==r || 'answer'==r) {                        
                        listAreaPrependTopic(r);
						if('answer'==r){$("#answer_" + i).remove();}
						// /listTopic(0,0);
					} else if( 'mycomment'==r ) {
                        window.location.href= thisSiteURL + 'index.php?mod=topic&code=myhome';
                    } else {
...
function onForwardSelectedListener(tid, options)
{
	...
	if($('#topicForwardType_'+tid).attr("checked") == true) {
...
{	
	...
	// 勾选 关注此用户 增加关注
	if($("#is_follow").attr("checked")==true){	
		// 触发关注js
		follow(touid,'','add');
	}
	var myAjax=$.post(
