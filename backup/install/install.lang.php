...
(
    ...
    'show_license' => 'DataCore系统 用户许可协议',
    'agreement_yes' => '我同意',
    'agreement_no' => '我不同意',
    ...
    'tablepre' => '表名前缀:',
    'tablepre_comment' => '默认无需修改，如同一数据库安装多个DataCore时必须修改',
    'tablepre_prompt' => '除非您需要在同一数据库安装多个 DataCore系统 \n,否则,强烈建议您不要修改表名前缀。',
    'recheck_config' => '重新检查设置',
    'check_env' => '检查当前服务器环境',
    'env_required' => 'DataCore系统 所需配置',
    'env_best' => 'DataCore系统 最佳配置',
    'env_current' => '当前服务器',
    'install_note' => '安装向导提示',
    'add_admin' => '设置管理员帐号',
    'start_install' => '开始安装 DataCore系统',
    ...
    'dbpriv_createtable' => '没有CREATE TABLE权限，无法安装DataCore系统',
    'dbpriv_insert' => '没有INSERT权限，无法安装DataCore系统',
    'dbpriv_select' => '没有SELECT权限，无法安装DataCore系统',
    'dbpriv_update' => '没有UPDATE权限，无法安装DataCore系统',
    'dbpriv_delete' => '没有DELETE权限，无法安装DataCore系统',
    'dbpriv_droptable' => '没有DROP TABLE权限，无法安装',
    'path_unsupport' => 'DataCore不支持在中文目录下安装',
    'php_version_406' => '您的 PHP 版本小于 4.0.6, 无法使用 DataCore系统。',
    ...
    'mysql_unsupport' => '您的服务器不支持MySql数据库，无法安装DataCore系统程序',
    ...
    'db_invalid' => '指定的数据库不存在, 系统也无法自动建立, 无法安装 DataCore系统。',
    'db_auto_created' => '指定的数据库不存在, 但系统已成功建立, 可以继续安装。',
    'db_not_null' => '数据库中已经安装过 DataCore系统, 继续安装会清空原有数据。',
    'db_drop_table_confirm' => '继续安装会清空全部原有数据，您确定要继续吗?',
    'install_in_processed' => '正在安装...',
    'install_succeed' => '恭喜您安装成功，点击进入网站首页',
    'license' => '<p class="subtitle">DataCore系统，以下简称DataCore或yqweibo。
<p>杭州神话信息技术有限公司为 DataCore 产品的开发商，依法独立拥有 DataCore 产品著作权。DataCore 官方网站网址为 http:/'.'/www.jishigou.net， 公司官方网址为 http:/'.'/www.cenwor.com。
<p>DataCore 著作权受法律和国际公约保护，使用者无论个人或组织、盈利与否、用途如何（包括以学习和研究为目的），均需仔细阅读本协议，在理解、同意、并遵守本协议的全部条款后，方可开始使用 DataCore 系统，杭州神话信息技术有限公司拥有对本授权协议的最终解释权。
<ul type="I">
<p><li><b>协议许可的权利</b>
<ul type="1">
    <li>您可以在完全遵守本最终用户授权协议的基础上，将本软件应用于非商业用途，而不必支付软件版权授权费用。 
    <li>您可以在协议规定的约束和限制范围内修改 DataCore 源代码(如果被提供的话)或界面风格以适应您的网站要求。 
...
<li>无论如何，即无论用途如何、是否经过修改或美化、修改程度如何，只要使用 DataCore 的整体或任何部分，未经书面许可或未购买去除DataCore版权信息的授权，页面页脚处的 yqweibo名称、官网链接（http:/'.'/www.jishigou.net）和杭州神话信息技术有限公司网站的链接（http:/'.'/www.INET.com） 都必须保留，而不能清除或修改。
<li>禁止在 DataCore 的整体或任何部分基础上以发展任何派生版本、修改版本或第三方版本用于重新分发。
...
<p>有关 DataCore系统 最终用户授权协议、商业授权与技术服务的详细内容，均由 DataCore 官方网站独家提供。CenWor Inc.拥有在不事先通知的情况下，修改授权协议和服务价目表的权力，修改后的协议或价目表对自改变之日起的新授权用户生效。
<p>电子文本形式的授权协议如同双方书面签署的协议一样，具有完全的和等同的法律效力。您一旦开始安装 DataCore，即被视为完全理解并接受
...
$msglang = array(
    'lock_exists' => '您已经安装过DataCore系统，如果您想重新安装，请删除 data/install.lock install/install.lock 文件，再次运行安装文件。',
    'short_open_tag_invalid' => '对不起，请将 php.ini 中的 short_open_tag 设置为 On，否则无法继续安装DataCore系统。',
