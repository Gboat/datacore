...
class ModuleObject extends MasterObject
{
        ...
        if ('myhome'==$params['code']) {
            ...
            if ($member['uid'] == MEMBER_ID) {
                ...
                if ($gid) {
                    ...
                    if($type || false === (Load::model('cache/db')->get($cache_key))) {
    ...
    function Top()
    {
        ...
        if ($limit>0 && false == ($day7_r_buddys = cache("misc/top_day7_r_buddys",$this->CacheConfig['topic_top']['renqi']))) {
            ...
            if($_ids) {
                $_list = $this->TopicLogic->GetMember($uids, "`uid`,`ucuid`,`username`,`fans_count`,`validate`,`validate_category`,`province`,`city`,`face`,`nickname`");
        ...
        if ($limit>0 && false == ($day7_r_users = cache("misc/top_day7_r_users",$this->CacheConfig['topic_top']['huoyue']))) {
            ...
            if ($day7_r_users) {
                $members = $this->TopicLogic->GetMember("where `uid` in ('".implode("','",$uids)."') limit {$limit}", "`uid`,`ucuid`,`username`,`fans_count`,`validate`,`validate_category`,`province`,`city`,`face`,`nickname`");
        ...
        if ($limit>0 && false == ($day7_r_topics = cache("misc/top_day7_r_topics",$this->CacheConfig['topic_top']['yingxiang']))) {
            ...
            if ($day7_r_topics) {
                $members = $this->TopicLogic->GetMember("where `face` !='' and `uid` in ('".implode("','",$uids)."') limit {$limit}","`uid`,`ucuid`,`username`,`fans_count`,`validate`,`validate_category`,`province`,`city`,`face`,`nickname`");
                foreach ($members as $_m) {
    ...
    function Hot()
    {
        ...
        if($Gz_limit > 0) {
            ...
            if(!$cache_time || (false === ($concern_users = cache('misc/topic-index-guanzhu-'.$Gz_limit, $cache_time)))) {
