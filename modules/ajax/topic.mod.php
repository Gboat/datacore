...
class ModuleObject extends MasterObject
{
    ...
    function DoList()
    {
        ...
        if ('myhome' == $this->Code) {
            ...
            $topic_uids[$uid] = $uid;
            if($is_personal) {
                if(false === (Load::model('cache/db')->get($cache_key))) {
    ...
    function DoAdd()
    {
        ...
        if('both' == $topic_type){
            ...
            $type = 'personal';
        } elseif(in_array($topic_type,array('answer','event','vote'))){
            $type = 'reply';
    ...
    function ViewComment($tid=0,$highlight=0,$return_reply='')
    {
        ...
        if($tid > 0)
        {
            ...
            if($topic_info)
            {
                ...
                {
                    rsort($tids);
                    $condition = "where `tid` in('".implode("','",array_slice((array) $tids,0,$limit))."')  order by `tid` desc limit {$limit}";
    ...
    function Modify_User_Signature()
    {
        ...
        $datas['signtime'] = 0;
        if($this->Config['sign_verify']){
            $count = $this->DatabaseHandler->ResultFirst("select count(*) from ".TABLE_PREFIX."members_verify where uid = $uid");
            if($count){
                $this->DatabaseHandler->Query("update ".TABLE_PREFIX."members_verify set signature = '$signature' , is_sign = 1 where uid = '$uid'");
            }else{
                $sql = "insert into ".TABLE_PREFIX."members_verify (uid,nickname,signature,is_sign) values ('$uid','".MEMBER_NICKNAME."','$signature',1)";
                $this->DatabaseHandler->Query($sql);
            }
            if($notice_to_admin = $this->Config['notice_to_admin']){
                $pm_post = array(
