 ...
 * @copyright 2010
 * @version $Id: user.mod.php 776 2012-04-20 03:44:12Z wushanghua $
 * @access public
 */
...
class ModuleObject extends MasterObject
{
	...
	function HotTag()
	{
		...
		if(time() - $hot_tag_recommend['time'] >= 30*60){
	...
	function getHotComment(){
		...
		if(!($topic_list = Load::model('cache/db')->get($cache_id))){
			$sql = "SELECT
					  `tid`,`content` 
			...
			while ($rs = DB::fetch($query)){
				$rs['content'] = cut_str($rs['content'],20);
				$topic_list[$rs['tid']] = $rs;
			}
			Load::model('cache/db')->set($cache_id,$topic_list,12*3600);
		}
	...
	function getHotTagTop(){
		...
		$tag_guanzu = array();
		$cache_id = 'hottagtop';
		if (!($tag_guanzu = Load::model('cache/db')->get($cache_id))) {
			$sql = "SELECT
			...
			Load::model('cache/db')->set($cache_id,$tag_guanzu,3600*12);
		}
	...
	function getVideo(){
		...
		if($buddys){
			...
			$sql = "select v.*,m.nickname,m.username from `".TABLE_PREFIX."topic_video` v 
					...
					where v.`uid` in (".implode(",",$buddys).") order by v.`id` desc limit 5";
			$query = DB::query($sql);
			while ($rs = DB::fetch($query)){
				$video_list[$rs['id']] = $rs;
			}
	...
	function getEventList(){
		...
		switch($this->Code){
						case 'buddys_create_event':
				if (!empty($buddys)) {
					$param['where'] = " a.postman in (".implode(",",$buddys).") and a.verify = 1 ";
				...
				break;
						case 'buddys_joined_event':
				if (!empty($buddys)) {
					$param['where'] = " m.play = 1 and m.fid in (".implode(",",$buddys).") ";
					$param['order'] = " order by a.lasttime desc,a.app_num desc,a.posttime desc ";	
					$param['limit'] = " LIMIT $limit ";
