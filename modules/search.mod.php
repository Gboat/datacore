...
class ModuleObject extends MasterObject
{
    ...
    function UserSearch()
    {
        ...
        if($usertag) {
            ...
            if(false === ($uids = Load::model('cache/db')->get($cache_key))) {
                $uids = array();
                ...
                Load::model('cache/db')->set($cache_key, $uids, $cache_time);
            }
            if($uids) {
                $where = " WHERE `uid` IN ('".implode("','",$uids)."') ";
        ...
        function TopicSearch()
    {
        ...
        if($keyword)
        {
            ...
            if($cache_time > 0) {
                if(false === ($tids = Load::model('cache/db')->get($cache_key))) {
                    $options = array(
                    ...
                    Load::model('cache/db')->set($cache_key, $tids, $cache_time);
                }
        ...
        function TagSearch()
    {
        ...
        if($tag) {
            ...
            if(false === ($topic_ids = Load::model('cache/db')->get($cache_key))) {
                ...
                $topic_ids = array();
                if($tag_id) {
                    $sql = "SELECT `item_id` FROM `".TABLE_PREFIX."topic_tag` WHERE `tag_id` in(".implode(",",$tag_id).") ORDER BY `item_id` DESC LIMIT {$this->cache_ids_limit} ";
                    $query = $this->DatabaseHandler->Query($sql);
                    while (false != ($row = $query->GetRow())) {
                        $topic_ids[$row['item_id']] = $row['item_id'];
                    }
                }
                Load::model('cache/db')->set($cache_key, $topic_ids, $cache_time);
            }
            if($topic_ids) {
    ...
    function VoteSearch()
    {
        ...
        if (!empty($q)) {
            ...
            if($cache_time > 0) {
                ...
                if(false === ($vids = Load::model('cache/db')->get($cache_key))) {
                    $param = array(
                        'where' => $where,
                        'order' => $order,
                        'count' => $this->cache_ids_limit,
                    ...
                    Load::model('cache/db')->set($cache_key, $vids, $cache_time);
                }
    ...
    function _MemberList($where='', $query_link='', $cache_time=0, $cache_key='') {
        ...
        if($cache_time > 0) {
            if(false === ($rets = Load::model('cache/db')->get($cache_key))) {
                ...
                $rets['uids'] = $uids;
                Load::model('cache/db')->set($cache_key, $rets, $cache_time);
            }
