 ...
 *
 * @Date 2012-04-28 05:53:11 291407329 1916100694 23356 $
 *******************************************************************/
...
class ModuleObject extends MasterObject
{
	...
	function getAllUser(){
		...
		$return = array();
		if($return = $EventLogic->getAllUser($param)){
			extract($return);
		}
	...
	function eventDetail(){
		...
		if(!$rs['verify'] || $rs['verify'] == 0){
			if($rs['postman'] != MEMBER_ID){
	    	    ...
	    	    $app_member_arr = $EventLogic->getAllUser(array('where'=>" a.id = '$id' and a.app = 1 and a.play = 0 ",'order'=>" order by a.app_time ",'limit'=>" limit 6 "));
	    $app_count = $app_member_arr['count'];
	    $app_member = $app_member_arr['member'];
	    
	    	    $play_member_arr = $EventLogic->getAllUser(array('where'=>" a.id = '$id' and a.play = 1  ",'order'=>" order by a.play_time ",'limit'=>" limit 6 "));
