...
class ModuleObject extends MasterObject
{
	...
	function delRecycling(){
		$query = $this->DatabaseHandler->Query("select `id`,`tid` from `".TABLE_PREFIX."topic_verify` where managetype = 1 limit 300");
		...
		if($tids){
			...
			$this->Messager('清空回收站成功','index.php?mod=topic&code=del&del=1');
		}
	}
