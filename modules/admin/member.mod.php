...
class ModuleObject extends MasterObject
{
    ...
    function DoSearch()
    {
        ...
        $invite_uid = (int) get_param('invite_uid');
          ...
          WHERE
              ...
              while (false != ($row = $query->GetRow()))
              {
                  ...
                  $memberfields[$row['uid']] = $row;
                            ...
                            $sql = "select `uid`, `nickname`, `username` from `".TABLE_PREFIX."members` where `invite_uid` in ('".implode("','", $invite_uids)."')";
              $query = $this->DatabaseHandler->Query($sql);
    ...
    function doAct(){
        ...
        if($act == 'sendmsg'){
            ...
            $PmLogic = new PmLogic();
            if($uids){
                $query = $this->DatabaseHandler->Query("select `nickname` from `".TABLE_PREFIX."members` where uid in (".implode(",",$uids).")");
                $nickname_arr = array();
                while (false != ($rs = $query->GetRow())){
