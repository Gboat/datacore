 ...
 *
 * @Date 2012-04-28 05:53:09 1390697813 1341157923 34970 $
 *******************************************************************/
...
class ModuleObject extends MasterObject
{
	...
	function Main()
	{
		...
		$wherelist =  "where VCF.is_audit = '". $this->Get['type'] ."'" ;
	...
	{	
	  	...
	  	$old_config = $this->Config;	  	$new_config = $old_config;
	  	$new_config['validate_people_setting'] = $data;
		ksort($new_config);
		ConfigHandler::set($new_config);
			...
			for ($i = 0; $i < count($uids); $i++) 
			{
				...
				if(empty($validate_info))
				{
					...
					$city_info = DB::result_first("select id from ".DB::table('common_district')." where `name`='".$member_info['city']."' ");
					 					...
					 					$sql = "update `".TABLE_PREFIX."members` set `validate_category` = '{$category_id}' where `uid`='{$uids[$i]}'";					
					$this->DatabaseHandler->Query($sql);
	...
	function uploadPic($field,$category_pic_id){
		...
		$image_name = $category_pic_id . "_o.jpg";
