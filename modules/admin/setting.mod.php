 ...
 * @version $Id: setting.mod.php 886 2012-04-28 05:32:04Z wushanghua $
 */
	...
	function DoModifyCreditsRule()
	{
		...
		if ($rid>0)
		{
			...
			function _saveRewriteConfig($domain,$name,$config)
			{
				...
				$fp= @fopen($file,"wb");
				if($fp==false)$this->Messager("无法写入配置文件 setting/rewrite.php");
				if($name=='settings')
				...
				if($_slide['list']) {
					foreach($_slide['list'] as $v) {
						if($v['code'] && !in_array($v['code'], $dels)) {
							$v['order'] = max(0, (int) $v['order']);
							$v['avaliable'] = $v['avaliable'] ? $v['avaliable'] : 0;
							$slide_order_max = max($slide_order_max,$v['order']);
							$slide_list[$v['code']] = $v;
						}
					}
				...
				if(!empty($sn_codes)) {
					foreach( $sn_codes as $key => $v) {
						...
						$slide_list[$v] = array(
		            		...
		            		'avaliable' => $sn_avaliable[$key] ? $sn_avaliable[$key] : 0,
		            		'type_list' => null,
						);
					}
				}
					...
					foreach( $tn_codes as $key => $subs) {
						foreach ($subs as $k => $v) {
							...
							$type_list[$key][$v] = array(
			            		...
			            		'avaliable' => $tn_avaliable[$key][$k] ? $tn_avaliable[$key][$k] : 0,
			            		'type_list' => null,
							);
						}
					}
