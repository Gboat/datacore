...
class ModuleObject extends MasterObject
{
    ...
    function Main() {
        ;
        $this->Messager(null, $this->Config['site_url'].'/index.php?mod=output&code=url_iframe&url='.$this->Config['site_url'].'&title=title&debug=1');
    }
    function UrlJs() {
        $hash = '';
        $info = array();
    ...
    function _input($var=null, $is_utf8=0, $ifemptyval=null) {
        $val = get_param($var);
        if($is_utf8) {
            $val = array_iconv('utf-8', $this->Config['charset'], $val);
        }
        if(!$val) {
            $val = $ifemptyval;
        }
