...
<?php } ?>
<div data-role="content" id="message_wp">
<script language="javascript">
<?php if($code == 404 || $code == 411) { ?>
        accessError(parseInt("<?php echo $code; ?>"));<?php } elseif($code == 400) { ?>showTips("没有数据", 0);<?php } elseif($code == 410) { ?>goToLogin();<?php } elseif($code == 300) { ?>showTips("用户不存在或者已经被删除了", 0);
<?php } else { ?>showTips("未知错误", 0);
<?php } ?>
</script>
</div>
<?php if(!$this->Config['is_mobile_client']) { ?>
