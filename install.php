 ...
 *
 * @Date 2012-04-28 05:53:05 1666933370 1891088808 27340 $
 *******************************************************************/
...
if(!defined('INSTALL_LANG') || !function_exists('instmsg') || !is_readable($sqlfile)) {
	...
	} elseif(is_file($lockfile) || is_file('./install/install.lock')) {
	instmsg('lock_exists');
...
<tr><td><?php echo $JISHIGOU_license; ?></td></tr>
</table></td></tr></table>
</td></tr>
<tr><td align="center">
...
<td class="altbg1">./data/errorlog</td>
<td class="altbg2"><?php echo $lang['writeable']; ?></td>
...
<?php
	...
	if($_POST['saveconfig']) {
		...
		$config['auth_key'] = setconfig(random(32)); 		$config['safe_key'] = setconfig(random(32));
			...
			if(!$write_error) {
				...
				if(!isset($config['link_list']['biniu'])) {
					$biniu_link_list = array(
						...
						array (
						    'name' => _getaarrayrandval(array('alexa排名','alexa排名查询','alexa网站排名')),
						    'url' => 'http:/'.'/alexa.biniu.com',
						),
						array (
						    'name' => _getaarrayrandval(array('同IP网站','同ip网站查询','同ip站点查询','ip反查域名','同ip网站查询')),
...
<td class="altbg2"><input type="text" name="db_name" value="<?php echo $config['db_name']; ?>" <?php echo $inputreadonly; ?> size="30"></td>
...
<td class="altbg2" width="80%">&nbsp;<input type="text" name="username" value="admin" size="30">（注意：创始人有最大的管理权限）</td>
</tr><tr>
...
$extrasql = <<<EOT
replace into `jishigou_members`(`uid`,`username`,`nickname`,`password`,`email`,`role_id`,`role_type`,`invitecode`, `regdate`, `lastactivity`, `regip`, `lastip`) values (1,'1','$username','$password','$email',2,'admin','{$invitecode}', '{$timestamp}', '{$timestamp}', '{$_SERVER['REMOTE_ADDR']}', '{$_SERVER['REMOTE_ADDR']}');
